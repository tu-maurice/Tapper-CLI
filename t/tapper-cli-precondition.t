...
my @retval = `/usr/bin/env perl -Ilib bin/tapper-testrun listprecondition --all`;
chomp @retval;
is_deeply(\@retval, [5, 6, 7, 8, 9, 10, 11, 101, 102, 103], 'List preconditions / all, short');

my $retval = `/usr/bin/env perl -Ilib bin/tapper-testrun listprecondition --id=5 -v`;
is($retval, "5 | temare_producer | ---\nprecondition_type: produce\nproducer: Temare\nsubject: KVM\nbitness: 64\n | NULL | \n", 'List preconditions / id, long');

@retval = `/usr/bin/env perl -Ilib bin/tapper-testrun listprecondition --testrun=3002`;
chomp @retval;
is_deeply(\@retval, [5, 8], 'List preconditions / per testrun, short');

@retval = `/usr/bin/env perl -Ilib bin/tapper-testrun updateprecondition  --shortname="foobar-perl-5.11" --condition="precondition_type: file\nname: some_file" 2>&1`;
chomp @retval;
