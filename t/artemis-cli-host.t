...
$host_id = `/usr/bin/env perl -Ilib bin/artemis-testrun newhost  --name="host2" --active --queue=KVM`;
chomp $host_id;
...
$host_id = `/usr/bin/env perl -Ilib bin/artemis-testrun newhost  --name="host3" --queue=Xen --queue=KVM`;
chomp $host_id;
...
# --------------------------------------------------
$host_id = qx(/usr/bin/env perl -Ilib bin/artemis-testrun newhost  --name="host4" --queue=noexist 2>&1);
like($host_id, qr(No such queue: noexist), 'Error handling for nonexistent queue');


# --------------------------------------------------
my $hosts = qx(/usr/bin/env perl -Ilib bin/artemis-testrun listhost --queue=KVM 2>&1);
like($hosts, qr(11 *| *host2\n *12 *| *host3\n), 'Show hosts / queue');


# --------------------------------------------------
qx(/usr/bin/env perl -Ilib bin/artemis-testrun updatehost --delqueue --active --id=7 2>&1);
